...
// Get supervisors (could eventually filter by admin)
$supervisores = getAdminSupervisors($user['id']);
...
if ($informeId) {
    $stmt = $pdo->prepare("SELECT * FROM informes_admin WHERE id = ? AND admin_id = ?");
    $stmt->execute([$informeId, $user['id']]);
    $informe = $stmt->fetch();
    if ($informe) {
        $stmt = $pdo->prepare("SELECT * FROM asistencia_admin WHERE informe_id = ?");
        $stmt->execute([$informeId]);
        while ($row = $stmt->fetch()) {
            $asistencias[$row['supervisor_id']] = $row;
        }
    }
} else {
    $stmt = $pdo->prepare("SELECT * FROM informes_admin WHERE admin_id = ? AND semana_inicio = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $totalSup = count($supervisores);
    $totalAsistentes = 0;
    foreach ($supervisores as $s) {
        $gf = $_POST['asistencia'][$s['id']]['grupo_familiar'] ?? 'no';
        if ($gf === 'si') $totalAsistentes++;
    }
    
    if ($informeId) {
        $stmt = $pdo->prepare("UPDATE informes_admin SET 
                ...
                estado=?, updated_at=NOW() WHERE id=?");
        $stmt->execute([
            $anfitrion, $fechaReunion, $direccion, $horaInicio, $horaTermino,
            $totalSup, $totalAsistentes, $nuevosEvangelizados,
            $confesionesFe, $temaExpuesto, $observaciones, $ofrenda, $proximaFechaOmt,
            $estado, $informeId
        ]);
    } else {
        $stmt = $pdo->prepare("INSERT INTO informes_admin 
            (admin_id, semana_inicio, semana_fin, anfitrion, fecha_reunion, direccion, hora_inicio, hora_termino,
    ...
    foreach ($supervisores as $s) {
        $grupoFamiliar = $_POST['asistencia'][$s['id']]['grupo_familiar'] ?? 'no';
        $escuela = $_POST['asistencia'][$s['id']]['escuela'] ?? 'no';
        $reunionRed = $_POST['asistencia'][$s['id']]['reunion_red'] ?? 'no';
        $cultoDomingo = $_POST['asistencia'][$s['id']]['culto_domingo'] ?? 'no';
        $actividadOmt = $_POST['asistencia'][$s['id']]['actividad_omt'] ?? 'no';
        $fechaLlamada = $_POST['asistencia'][$s['id']]['fecha_llamada'] ?? null;
        $notaConsolidacion = (int)($_POST['asistencia'][$s['id']]['nota_consolidacion'] ?? 0);
        $attendance = [
        ...
        $stmt = $pdo->prepare("SELECT id FROM asistencia_admin WHERE informe_id = ? AND supervisor_id = ?");
        $stmt->execute([$informeId, $s['id']]);
        if ($stmt->fetch()) {
            $stmt = $pdo->prepare("UPDATE asistencia_admin SET 
                ...
                WHERE informe_id=? AND supervisor_id=?");
            $stmt->execute([
                $grupoFamiliar, $escuela, $reunionRed, $cultoDomingo, $actividadOmt,
                $porcentaje, $fechaLlamada, $notaConsolidacion, $informeId, $s['id']
            ]);
        } else {
            $stmt = $pdo->prepare("INSERT INTO asistencia_admin 
                (informe_id, supervisor_id, grupo_familiar, escuela, reunion_red, culto_domingo, 
                 actividad_omt, porcentaje_asistencia, fecha_llamada_visita, nota_consolidacion)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute([
                $informeId, $s['id'], $grupoFamiliar, $escuela, $reunionRed, $cultoDomingo,
    ...
    if ($estado === 'completado') {
        generateMonthlyStatsAdmin($user['id'], date('n'), date('Y'));
    }
    logActivity($user['id'], 'informe_admin', "Informe admin " . ($estado === 'completado' ? 'completado' : 'guardado'));
    $success = $estado === 'completado' ? '¡Informe completado exitosamente!' : 'Informe guardado como borrador.';
    
    $stmt = $pdo->prepare("SELECT * FROM informes_admin WHERE id = ?");
    $stmt->execute([$informeId]);
    $informe = $stmt->fetch();
...
$pageTitle = 'Informe Semanal de Administrador';
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<body>
    ...
    <div class="admin-content">
        ...
        <div class="alert alert-warning">
            ...
            No tienes supervisores asignados.
        </div>
        <?php else: ?>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $s['id'] ?>][grupo_familiar]" value="<?= $att['grupo_familiar'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $s['id'] ?>][escuela]" value="<?= $att['escuela'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $s['id'] ?>][reunion_red]" value="<?= $att['reunion_red'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $s['id'] ?>][culto_domingo]" value="<?= $att['culto_domingo'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $s['id'] ?>][actividad_omt]" value="<?= $att['actividad_omt'] ?? 'no' ?>">
