...
// identify which type of report we're viewing (supervisor vs líder)
...
// try fetching a supervisor report first
$stmt = $pdo->prepare("SELECT i.*, u.full_name as supervisor_nombre
...
if ($informe) {
    ...
    // fall back to líder report
    $stmt = $pdo->prepare("SELECT i.*, u.full_name as lider_nombre
...
if ($user['role'] === 'supervisor') {
    if ($isSupervisorReport) {
        // supervisor can only see their own reports
        if ($informe['supervisor_id'] !== $user['id']) {
            header('Location: ' . BASE_URL . '/supervisor/');
            exit;
        }
    } else {
        // supervisor viewing a líder report - ensure leader belongs to them
        $assignedLeaders = getSupervisorLeaders($user['id']);
        $leaderIds = array_column($assignedLeaders, 'id');
        if (!in_array($informe['lider_id'], $leaderIds)) {
            header('Location: ' . BASE_URL . '/supervisor/');
            exit;
        }
    }
...
// prepare labels for display now that $informe is guaranteed
$personLabel = $isSupervisorReport ? 'Supervisor' : 'Líder';
$personName = $isSupervisorReport ? $informe['supervisor_nombre'] : $informe['lider_nombre'];

// Get attendance records
if ($isSupervisorReport) {
    $stmt = $pdo->prepare(
        "SELECT a.*, u.full_name as lider_nombre
         FROM asistencia_supervisores a
         JOIN users u ON a.lider_id = u.id
         WHERE a.informe_id = ?"
    );
} else {
    $stmt = $pdo->prepare(
...
<body>
    ...
    <div class="admin-content">
        <div class="admin-header no-print">
            <div>
                <h1><?= $pageTitle ?></h1>
                <p class="text-muted mb-0"><?= $personLabel ?>: <?= sanitize($personName) ?></p>
            </div>
            <div class="d-flex gap-2">
                <button onclick="window.print()" class="btn btn-outline-primary">
                    <i class="bi bi-printer me-2"></i>Imprimir
                </button>
                <a href="<?= BASE_URL ?>/supervisor/" class="btn btn-outline-secondary">
        ...
        <div class="card border-0 shadow-sm mb-4">
            ...
            <div class="card-body p-0">
                <div class="px-3 py-2 small text-muted">N/H = No Hubo (no se considera para el porcentaje; 0% sólo si se marca "No")</div>
                <div class="table-responsive">
                    <table class="table table-bordered mb-0">
                        <thead class="bg-light">
                            <tr>
                                <th><?= $isSupervisorReport ? 'Líder' : 'Miembro' ?></th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <?= sanitize($isSupervisorReport ? $att['lider_nombre'] : $att['miembro_nombre']) ?>
                                    <?php if (!$isSupervisorReport && !empty($att['es_consolidacion'])): ?>
                                    <span class="badge bg-warning ms-1">C</span>
                                    <?php elseif ($isSupervisorReport && !empty($att['nota_consolidacion'])): ?>
                                    <span class="badge bg-warning ms-1">C</span>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="text-center fw-bold"><?= round($att['porcentaje_asistencia']) ?>%</td>
                            </tr>
                            <?php endforeach; ?>
