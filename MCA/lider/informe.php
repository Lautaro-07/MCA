...
if ($informeId) {
    $stmt = $pdo->prepare("SELECT * FROM informes_semanales WHERE id = ? AND lider_id = ?");
    $stmt->execute([$informeId, $user['id']]);
    $informe = $stmt->fetch();
    
    if ($informe) {
        // Get attendance records
        $stmt = $pdo->prepare("SELECT * FROM asistencia_semanal WHERE informe_id = ?");
        $stmt->execute([$informeId]);
        while ($row = $stmt->fetch()) {
            $asistencias[$row['miembro_id']] = $row;
        }
    }
} else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Calculate totals
    $enConsolidacion = count(array_filter($miembros, fn($m) => $m['es_consolidacion']));
    $totalMiembros = count($miembros);
    $totalAsistentes = 0;
    
    // Count attendance
    foreach ($miembros as $m) {
        $gf = $_POST['asistencia'][$m['id']]['grupo_familiar'] ?? 'no';
    ...
    if ($informeId) {
        ...
        $stmt = $pdo->prepare("
            UPDATE informes_semanales SET 
                anfitrion = ?, fecha_reunion = ?, direccion = ?, hora_inicio = ?, hora_termino = ?,
                total_miembros = ?, en_consolidacion = ?, total_asistentes = ?, nuevos_evangelizados = ?,
        ...
        $stmt->execute([
            ...
            $totalMiembros, $enConsolidacion, $totalAsistentes, $nuevosEvangelizados,
        ...
        $stmt = $pdo->prepare("
            ...
            (lider_id, semana_inicio, semana_fin, anfitrion, fecha_reunion, direccion, hora_inicio, hora_termino,
             total_miembros, en_consolidacion, total_asistentes, nuevos_evangelizados, confesiones_fe,
    ...
    foreach ($miembros as $m) {
        $grupoFamiliar = $_POST['asistencia'][$m['id']]['grupo_familiar'] ?? 'no';
        $escuela = $_POST['asistencia'][$m['id']]['escuela'] ?? 'no';
        $reunionRed = $_POST['asistencia'][$m['id']]['reunion_red'] ?? 'no';
        $cultoDomingo = $_POST['asistencia'][$m['id']]['culto_domingo'] ?? 'no';
        $actividadOmt = $_POST['asistencia'][$m['id']]['actividad_omt'] ?? 'no';
        $fechaLlamada = $_POST['asistencia'][$m['id']]['fecha_llamada'] ?? null;
        $notaConsolidacion = (int)($_POST['asistencia'][$m['id']]['nota_consolidacion'] ?? 0);
        ...
        $stmt = $pdo->prepare("SELECT id FROM asistencia_semanal WHERE informe_id = ? AND miembro_id = ?");
        $stmt->execute([$informeId, $m['id']]);
        
        if ($stmt->fetch()) {
            $stmt = $pdo->prepare("
                UPDATE asistencia_semanal SET 
                ...
                WHERE informe_id = ? AND miembro_id = ?
            ");
            $stmt->execute([
                $grupoFamiliar, $escuela, $reunionRed, $cultoDomingo, $actividadOmt,
                $porcentaje, $fechaLlamada, $notaConsolidacion, $informeId, $m['id']
            ]);
        } else {
            $stmt = $pdo->prepare("
                INSERT INTO asistencia_semanal 
                (informe_id, miembro_id, grupo_familiar, escuela, reunion_red, culto_domingo, 
                 actividad_omt, porcentaje_asistencia, fecha_llamada_visita, nota_consolidacion)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $informeId, $m['id'], $grupoFamiliar, $escuela, $reunionRed, $cultoDomingo,
                $actividadOmt, $porcentaje, $fechaLlamada, $notaConsolidacion
            ]);
        }
    }
    
    // Generate monthly stats if completing report
    if ($estado === 'completado') {
        generateMonthlyStats($user['id'], date('n'), date('Y'));
    }
    
    logActivity($user['id'], 'informe_semanal', "Informe semanal " . ($estado === 'completado' ? 'completado' : 'guardado'));
    
    if ($estado === 'completado') {
        $success = '¡Informe completado exitosamente!';
    } else {
        $success = 'Informe guardado como borrador.';
    }
    
    // Refresh data
    $stmt = $pdo->prepare("SELECT * FROM informes_semanales WHERE id = ?");
    $stmt->execute([$informeId]);
    $informe = $stmt->fetch();
...
<body>
    ...
    <div class="admin-content">
        ...
        <div class="alert alert-warning">
            ...
            No tienes miembros registrados. <a href="<?= BASE_URL ?>/lider/miembros.php">Agregar miembros</a> antes de crear un informe.
        </div>
        <?php else: ?>
            ...
            <div class="report-section">
                ...
                <div class="table-responsive">
                    <table class="attendance-table">
                        <thead>
                            <tr>
                                <th style="text-align: left; width: 200px;">Miembro</th>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $m['id'] ?>][grupo_familiar]" value="<?= $att['grupo_familiar'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $m['id'] ?>][escuela]" value="<?= $att['escuela'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $m['id'] ?>][reunion_red]" value="<?= $att['reunion_red'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $m['id'] ?>][culto_domingo]" value="<?= $att['culto_domingo'] ?? 'no' ?>">
                                ...
                                <td>
                                    <div class="attendance-toggle">
                                        <input type="hidden" name="asistencia[<?= $m['id'] ?>][actividad_omt]" value="<?= $att['actividad_omt'] ?? 'no' ?>">
                                ...
                                <td>
                                    <span class="badge <?= $m['esta_bautizado'] ? 'bg-success' : 'bg-secondary' ?>">
                                        <?= $m['esta_bautizado'] ? 'Sí' : 'No' ?>
                                    </span>
                                </td>
